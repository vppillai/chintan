...
// append markers removed, chillu spellings folded (ask.FoldScript), lowercased,
// whitespace collapsed, and cut to model.MaxSearchTextBytes on a rune boundary.
//
...
func SearchText(body string) string {
	text := strings.ToLower(ask.FoldScript(StripCaptureMarkers(body)))
