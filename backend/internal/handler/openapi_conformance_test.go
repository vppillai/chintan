...
func statusScenarios() map[string]scenario {
	...
	return map[string]scenario{
		...
		"POST /v1/inbox/audio -> 400": func(t *testing.T) int {
			// Four roads to it: a raw body that is not audio, a form with
			// neither part the route reads, a form whose audio is not audio,
			// and a form whose audio part has no type. The first that is not
			// a 400 is the answer.
			h := newHarness(t)
			key := h.deviceKey(t)
			emptyForm, emptyType := ringForm(t, nil, "", "")
			notAudio, notAudioType := ringForm(t, []byte("not audio"), "text/plain", "")
			untyped, untypedType := ringForm(t, []byte("audio bytes"), "", "")
			for _, code := range []int{
				...
				h.do(t, http.MethodPost, "/v1/inbox/audio", "", untyped, key, [2]string{"Content-Type", untypedType}).Code,
			} {
