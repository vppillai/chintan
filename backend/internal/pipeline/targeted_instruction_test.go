...
// A recording into a checklist is not stripped by the router at all: the
// item extraction reads the raw transcript and handles the words addressed to
// the app itself, so "Add umbrella to shopping list" — which no instruction
// cue catches and which used to be appended verbatim — is the item Umbrella,
// and no routing-priced call is made.
func TestARecordingIntoAChecklistMakesNoRouterCallAndYieldsItems(t *testing.T) {
	...
	if got := h.router.CallCount(); got != 0 {
		t.Errorf("router calls = %d, want none: the extraction handles the instruction words", got)
	}
	body, _ := h.objects.Get(ctx, "tenants/user1/notes/note1/note.md")
	if string(body) != service.CaptureMarker("c_1")+"\n- [ ] Umbrella" {
		t.Errorf("body = %q", body)
	}
	if calls := h.llm.ItemsCalls(); len(calls) != 1 || calls[0].Transcript != "Add this to my list. Add umbrella to shopping list" {
		t.Errorf("extraction calls = %+v, want one over the raw transcript", calls)
	}
}
