...
#
# What it creates, all named voicenotes-*:
#
#   voicenotes-agent-boundary     managed policy — the permissions CEILING
#   voicenotes-agent-deny         managed policy — explicit denies, override any allow
#   voicenotes-agent-permissions  managed policy — what the agent may actually do
#   voicenotes-agent             IAM role, boundary attached, both policies attached
#   voicenotes-agent-cli         IAM user, boundary attached, may only assume that role
#   voicenotes-cloudtrail-*      S3 bucket for the audit trail
#   voicenotes-trail             CloudTrail trail
#
...
#
#   voicenotes-agent-cli   an IAM user whose ONLY permission is sts:AssumeRole on the
#                          role below, with the boundary also attached so it can never
#                          be granted more. Holds the one long-lived access key.
#   voicenotes-agent       the role that actually carries the project permissions.
#
...
#   scripts/bootstrap-agent.sh --region <r>    # default: ca-central-1
#
...
ROLE_NAME="voicenotes-agent"
BOUNDARY_NAME="voicenotes-agent-boundary"
DENY_NAME="voicenotes-agent-deny"
PERMS_NAME="voicenotes-agent-permissions"
TRAIL_NAME="voicenotes-trail"
USER_NAME="voicenotes-agent-cli"
...
TRAIL_BUCKET="voicenotes-cloudtrail-${ACCOUNT_ID}-${REGION}"
...
dim ""
dim "  nothing outside the voicenotes-* namespace is touched."
...
TAGS=(
    ...
    Key=CostCenter,Value=voicenotes-shared
)
...
create_policy() {
    ...
    if exists_policy "$name"; then
        ...
        aws iam create-policy-version \
            ...
            --policy-document "file://${RENDER_DIR}/${name#voicenotes-agent-}.json" \
...
create_policy "$PERMS_NAME" "Chintan agent grants, scoped to voicenotes-* resources in the deploy region."
...
aws s3api put-bucket-tagging --bucket "$TRAIL_BUCKET" --tagging \
    'TagSet=[{Key=Project,Value=voicenotes},{Key=Instance,Value=shared},{Key=Environment,Value=shared},{Key=ManagedBy,Value=iac},{Key=Owner,Value=vppillai},{Key=CostCenter,Value=voicenotes-shared},{Key=Protected,Value=true}]' >/dev/null
...
if ! exists_trail "$TRAIL_NAME"; then
    ...
    aws cloudtrail create-trail \
        ...
        --tags-list "Key=Project,Value=voicenotes" "Key=Instance,Value=shared" "Key=ManagedBy,Value=iac" >/dev/null
