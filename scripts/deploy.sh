...
if [ "${NO_CHANGES:-0}" != "1" ]; then
    ...
    # After every deploy, not only a CREATE: a stack that bootstrap.sh created
    # before it protected stacks is covered by its next update rather than
    # never (see protect_stack in scripts/lib/common.sh).
    protect_stack "$STACK"
    ok "$STACK deployed"
elif ! is_apply; then
    exit 0
